#ifndef SCENE_H
#define SCENE_H

#include "raylib.h"

// Enum definition for the different game screens
typedef enum GameScreen { LOGO = 0, TITLE, GAMEPLAY, ENDING } GameScreen;

// Function declarations for updating and drawing each screen dunno why it ain't workin' like i was expectin' lol
//void UpdateDrawLogoScreen(GameScreen *currentScreen);
//void UpdateDrawTitleScreen(GameScreen *currentScreen);
//void UpdateDrawGameplayScreen(GameScreen *currentScreen, TileManager *tileManagers);
//void UpdateDrawEndingScreen(GameScreen *currentScreen);

#endif // SCENE_H
